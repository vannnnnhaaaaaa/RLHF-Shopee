...
import './style.scss';
import { create_new_product, get_product_detail, update_product } from '../../../services/product';

// ĐỊNH NGHĨA DOMAIN CỦA BACKEND ĐỂ HIỂN THỊ ẢNH
...
const ProductForm = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', category: '', description: '', has_variants: false,
  ...
  // --- 2. KÉO DỮ LIỆU CŨ KHI Ở CHẾ ĐỘ EDIT ---
  useEffect(() => {
    if (isEditMode) {
      const fetchOldData = async () => {
        ...
        try {
          ...
          // [ĐÃ SỬA]: FIX LỖI HIỂN THỊ ẢNH TỪ DATABASE
         // [ĐÃ SỬA]: XỬ LÝ ẢNH TỪ SUPABASE
          if (product.images && product.images.length > 0) {
            
            // Dòng này để bạn mở F12 (Console) lên xem Backend đang trả về tên cột là gì
            console.log("DỮ LIỆU ẢNH TỪ BACKEND:", product.images);

            const oldImages = product.images.map(img => {
              // Phòng hờ Backend đặt tên cột là url thay vì image_link
              const actualUrl = img.image_link || img.url || img.image_url; 
              
              return {
                isExisting: true,
                id: img.id,
                previewUrl: actualUrl // Bỏ thẳng link Supabase vào, không cần cộng domain!
              };
            });
            setImages(oldImages);
          }
          ...
          if (product.video_link) {
            const videoUrl = product.video_link.startsWith('http') 
                                ? product.video_link 
                                : `${API_URL}${product.video_link}`;
            setVideo({ isExisting: true, previewUrl: videoUrl });
          }
          ...
          if (product.has_variants && product.skus && product.skus.length > 0) {
            ...
            if (product.skus[0].tier_1_name) {
              const options1 = [...new Set(product.skus.map(s => s.tier_1_value).filter(Boolean))];
              loadedTiers.push({ name: product.skus[0].tier_1_name, options: options1 });
            }
            if (product.skus[0].tier_2_name) {
              const options2 = [...new Set(product.skus.map(s => s.tier_2_value).filter(Boolean))];
              loadedTiers.push({ name: product.skus[0].tier_2_name, options: options2 });
            }
            setTiers(loadedTiers.length > 0 ? loadedTiers : [{ name: '', options: [] }]);

            const loadedVariants = product.skus.map(s => ({
              id: s.id,
              tier1: s.tier_1_value || '',
              tier2: s.tier_2_value || null,
              price: s.price || '',
              stock: s.stock || '',
              // Đã Xóa bỏ trường sku (Mã SKU người bán)
            }));
          ...
          navigate('/seller-dashboard/products/list');
        } finally {
  ...
  useEffect(() => {
    if (!hasVariants || loadingData) return;
    ...
    setVariantsList(prev => {
      return combos.map(combo => {
        ...
        // [ĐÃ SỬA]: Xóa trường sku khỏi object default
        return existing ? existing : { ...combo, price: '', stock: '' }; 
      });
    });
  }, [tiers, hasVariants, loadingData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { 
        formData, hasVariants, variantsList, tiers, 
        newImages, existingImages, video 
      };

      if (isEditMode) {
        await update_product(id, payload);
        alert("Cập nhật sản phẩm thành công!");
      } else {
        await create_new_product(payload);
        alert("Thêm sản phẩm thành công!");
      }
      
      navigate('/seller-dashboard/products/manage'); 
    } catch (error) {
  ...
  if (loadingData) {
    return <div style={{ padding: '50px', textAlign: 'center', fontSize: '18px' }}>⏳ Đang tải thông tin sản phẩm...</div>;
  }
          ...
          <section id="basic-info" className="content-card">
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Nhập tên sản phẩm..." maxLength="255" required />
          ...
          <section id="product-details" className="content-card">
            ...
            <div className="form-group">
              ...
              <div className="editor-container">
                <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Nhập mô tả sản phẩm..." rows="8" required></textarea>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              {video ? (
                <div style={{ position: 'relative', width: '200px' }}>
                  ...
                  <button type="button" onClick={removeVideo} style={{ position: 'absolute', top: -10, right: -10, background: 'red', color: 'white', border: 'none', borderRadius: '50%', cursor: 'pointer', padding: '5px 10px' }}>
                    X
                  </button>
                </div>
              ) : (
          ...
          <section id="sales-info" className="content-card">
            ...
            {!hasVariants && (
              <div className="form-row mt-20">
                <div className="form-group half">
                  ...
                  <input type="number" name="price" value={formData.price} onChange={handleChange} placeholder="Nhập giá" required={!hasVariants} />
                </div>
                <div className="form-group half">
                  <label className="required">Hàng có sẵn</label>
                  <input type="number" name="stock" value={formData.stock} onChange={handleChange} placeholder="Nhập số lượng" required={!hasVariants} />
                </div>
              </div>
            )}
            ...
            {hasVariants && (
              <div className="variant-builder mt-20">
                  ...
                  <div key={tIndex} className="tier-box">
                    ...
                    <div className="tier-body">
                      <div className="form-group mb-15">
                        <input type="text" placeholder="Kích thước, Màu sắc..." value={tier.name} onChange={(e) => handleTierNameChange(tIndex, e.target.value)} />
                      </div>
                      <div className="form-group mb-0">
                        ...
                        <div className="options-input-wrapper">
                          ...
                          <input type="text" placeholder="Thêm giá trị khác (Nhấn Enter để thêm)" onKeyDown={(e) => handleAddOption(e, tIndex)} />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {tiers.length < 2 && (
                  <button type="button" className="btn-add-tier" onClick={addTier}>+ Thêm biến thể</button>
                )}
                    ...
                    <table className="variant-table">
                      <thead>
                        <tr>
                          ...
                          {/* [ĐÃ SỬA]: Gỡ bỏ cột SKU người bán */}
                      ...
                      <tbody>
                        {variantsList.map((variant, vIndex) => (
                          <tr key={vIndex}>
                            ...
                            <td>
                              <input type="number" value={variant.price} onChange={(e) => handleVariantChange(vIndex, 'price', e.target.value)} placeholder="đ" required />
                            </td>
                            <td>
                              <input type="number" value={variant.stock} onChange={(e) => handleVariantChange(vIndex, 'stock', e.target.value)} placeholder="0" required />
                            </td>
                          </tr>
                        ))}
                      </tbody>
          ...
          <section id="shipping-info" className="content-card">
            ...
            <div className="form-group">
              ...
              <div className="weight-input-wrapper">
                ...
                <input type="number" name="weight" value={formData.weight} onChange={handleChange} placeholder="Nhập trọng lượng" required />
              </div>
            </div>
