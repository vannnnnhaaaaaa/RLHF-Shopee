...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. Validation (Giữ logic kiểm tra tại UI để phản hồi nhanh)
    if (images.length < 5) {
    ...
    // 2. Gọi Service
    try {
      // Gom tất cả dữ liệu cần thiết gửi sang Service
      const payload = { formData, hasVariants, variantsList, tiers, images, video };
      const result = await create_new_product(payload);
