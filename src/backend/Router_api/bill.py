...
from src.backend.services.bill_service import BillService 
...
@router_bill.post("/", response_model=ResponseBill)
async def create_bill_endpoint(
    ...
    bill_data.customer_id = current_user.id
    print(bill_data)
    try:
        # 3. Bắt đầu tạo hóa đơn
        result = await BillService.create_bill(bill_data=bill_data, session=session)
        return result
